	...
	doc.on('click', '.navigable li, .navigable li > span, .navigable li > a', function(event)
	{
		...
		if (back.length === 0)
		{
			...
			back = $('<div class="back"><span class="back-arrow"></span>Back</div>').prependTo(root).click(function(event)
			{
				...
				if (!current)
				{
					back.stop(true).animate({ marginTop: -back.outerHeight()+'px' });
					return;
				}
				...
				root.stop(true).animate({ height: (target.outerHeight(true)+backHeight)+'px' });

				// Move whole navigation to reveal target ul
				mainUL.stop(true).animate({ left: left+'%' });
			});
		...
		if (load.length > 0)
		{
			...
			mainUL.stop(true).animate({ left: -(current.parentsUntil('.navigable', 'ul').length*100)+'%' }, 'fast');
			load.stop(true).removeData('navigable-target').animate({ right: '-10%' }, 'fast', function()
			{
				load.remove();
				clicked.click();
			});
		...
		if (submenu.length > 0)
		{
			...
			if ($.inArray(li.css('position'), ['relative', 'absolute']) > -1)
			{
				submenu.css('top', -li.position().top+'px');
			}
			...
			// Set root element size according to target size
			root.stop(true).height(parentUL.outerHeight(true)+back.outerHeight(true)).animate({ height: (submenu.outerHeight(true)+back.outerHeight())+'px' });

			// Move whole navigation to reveal target ul
			mainUL.stop(true).animate({ left: -(allUL.length*100)+'%' });

			// Show back button
			back.animate({ marginTop: 0 });
		...
		else if (clicked.hasClass('navigable-ajax'))
		{
			...
			if (url && typeof url === 'string' && $.trim(url).length > 0 && url.substr(0, 1) !== '#')
			{
				...
				mainUL.stop(true).animate({ left: -((allUL.length-1)*100+10)+'%' });

				// Show load
				load.animate({ right: '0%' });
